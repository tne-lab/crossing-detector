...
void CrossingDetector::registerParameters()
{
    ...
    addIntParameter(Parameter::PROCESSOR_SCOPE, "Timeout_ms", "Timeout_ms", "Minimum length of time between consecutive events",
                    timeout, 0, 100000);

    addIntParameter(Parameter::PROCESSOR_SCOPE, "threshold_type", "Threshold type", "Type of Threshold to use", thresholdType, 0, 5);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "constant_threshold", "Constant threshold", "Constant threshold value", "mV",
                    constantThresh, -FLT_MAX, FLT_MAX, 0.1f); 
    

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "avg_decay_seconds", "avg_decay_seconds", "RMS Average Duration", "secs" , averageDecaySeconds, 0, FLT_MAX, 0.01f);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "min_random_threshold", "min_random_threshold", "Minimum random threshold value","mV",
                    randomThreshRange[0], -10000.0f, 10000.0f, 0.1f);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "max_random_threshold", "max_random_threshold", "Maximum random threshold value", "mV",
                    randomThreshRange[1], -10000.0f, 10000.0f, 0.1f);

    addIntParameter(Parameter::STREAM_SCOPE, "threshold_chan", "Threshold ref. chan", "Threshold reference channel", 0, 0, 1000);

    addIntParameter(Parameter::PROCESSOR_SCOPE, "past_span","past_span", "Number of past samples to look at at each timepoint (attention span)",
                    pastSpan, 0, 100000);

    addIntParameter(Parameter::PROCESSOR_SCOPE, "future_span", "future_span", "Number of future samples to look at at each timepoint (attention span)",
                    futureSpan, 0, 100000);
    
    addFloatParameter(Parameter::PROCESSOR_SCOPE, "past_strict", "past_strict", "fraction of past span required to be above / below threshold", "",
                    pastStrict, 0.0f, 1.0f, 0.01f);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "future_strict",  "future_strict", "fraction of future span required to be above / below threshold", "",
    ...
    addFloatParameter(Parameter::PROCESSOR_SCOPE, "jump_limit_sleep", "jump limit sleep", "Sleep after artifact", "ms",
    ...
    addIntParameter(Parameter::PROCESSOR_SCOPE, "buffer_end_mask", "buffer end mask", "Ignore crossings ocurring specified ms before the end of a buffer",
                    bufferEndMaskMs, 0, INT_MAX);

    addIntParameter(Parameter::PROCESSOR_SCOPE, "event_duration", "Event Duration", "Event Duration", eventDuration, 0, INT_MAX);
    
    // Adaptive threshold
    addIntParameter(Parameter::STREAM_SCOPE, "indicator_channel", "indicator channel", "Index of monitored event channel", -1, -1, INT_MAX);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "indicator_target", "indicator target", "Target indicator", "deg", indicatorTarget, FLT_MIN, FLT_MAX, 0.1f);

    addBooleanParameter(Parameter::PROCESSOR_SCOPE, "use_indicator_range", "use indicator range", "Whether to use circular range", useIndicatorRange);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "indicator_range_start", "indicator range start", "Start of indicator range", "deg", indicatorRange[0], FLT_MIN, FLT_MAX, 0.1f);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "indicator_range_end", "indicator range end", "End of indicator range", "deg", indicatorRange[1], FLT_MIN, FLT_MAX, 0.1f);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "start_learning_rate", "start learning rate", "Initial learning rate","", startLearningRate, FLT_MIN, FLT_MAX, 0.001f);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "min_learning_rate", "min learning rate", "Minimum learning rate","", minLearningRate, FLT_MIN, FLT_MAX, 0.0001f);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "decay_rate","decay rate", "Decay rate","", decayRate, FLT_MIN, FLT_MAX, 0.00001f);

    addBooleanParameter(Parameter::PROCESSOR_SCOPE, "adapt_threshold_paused", "adapt threshold paused", "Whether the adaptive threshold is paused", adaptThreshPaused);

    addBooleanParameter(Parameter::PROCESSOR_SCOPE, "use_adapt_threshold_range", "use adapt threshold range", "Whether to keep the threshold within the range", useAdaptThreshRange);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "adapt_threshold_range_start", "adapt threshold range start", "Start of adaptive threshold range", "deg",
        adaptThreshRange[0], FLT_MIN, FLT_MAX, 0.1f);

    addFloatParameter(Parameter::PROCESSOR_SCOPE, "adapt_threshold_range_end", "adapt threshold range end", "End of adaptive threshold range", "deg", adaptThreshRange[1], FLT_MIN, FLT_MAX, 0.1f);
}
...
void CrossingDetector::parameterValueChanged(Parameter* param)
{
    ...
    else if (param->getName().equalsIgnoreCase("Channel"))
    {
        ...
        static_cast<CrossingDetectorEditor*>(getEditor())->updateVisualizer();  // check what this does 
