...
void CrossingDetectorCanvas::initializeOptionsPanel()
{
    ...
    targetEditable = createEditable("indicator target", lastTargetEditableString,
        "deg", bounds = { xPos += 150, yPos, 80, C_TEXT_HT });
    ...
    restartButton = new UtilityButton("RESTART");
    restartButton->setFont(FontOptions(20.0f));
    ...
    pauseButton = new UtilityButton("PAUSE");
    pauseButton->setFont(FontOptions(20.0f));
    ...
    limitEditable = createEditable("LimitE", String((float)processor->getParameter("jump_limit")->getValue()), "mV",
    ...
    limitSleepEditable = createEditable("LimitSE", String((float)processor->getParameter("jump_limit_sleep")->getValue()), "ms",
        bounds = { xPos += 150, yPos, 50, C_TEXT_HT });
